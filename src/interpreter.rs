...
impl Interpreter {
                ...
                match (tv, iv) {
                    (Value::List(items), Value::Int(idx)) => {
                        ...
                        if real_idx < 0 || real_idx >= len {
                            return Err(error(
                                format!("index {idx} out of range (len={len})"),
                                span,
                            ));
                        }
                ...
                match (tv, iv) {
                    ...
                    (Value::String(s), Value::Int(idx)) => {
                        ...
                        Ok(Flow::Value(Value::String(
                            (bytes[real as usize] as char).to_string(),
                        )))
                    }
                    (Value::Dict(map), Value::String(key)) => {
        ...
        match builtin {
            ...
            Builtin::Abs => {
                ...
                match &args[0] {
                    ...
                    other => Err(error(
                        format!("abs: expected Int or Float, got {other}"),
                        span,
                    )),
                }
            }
                ...
                if args.len() == 1 {
                    ...
                    if let Value::List(items) = &args[0] {
                        ...
                        return items
                            .iter()
                            .cloned()
                            .try_fold(items[0].clone(), |acc, v| numeric_cmp_min(acc, v, span));
                    }
                }
                if args.len() < 2 {
                    return Err(error("min expects 2+ arguments or a list", span));
                }
                args.into_iter().try_fold(Value::Unit, |acc, v| {
                    if matches!(acc, Value::Unit) {
                        Ok(v)
                    } else {
                ...
                if args.len() == 1 {
                    if let Value::List(items) = &args[0] {
                        ...
                        return items
                            .iter()
                            .cloned()
                            .try_fold(items[0].clone(), |acc, v| numeric_cmp_max(acc, v, span));
                    }
                }
                if args.len() < 2 {
                    return Err(error("max expects 2+ arguments or a list", span));
                }
                args.into_iter().try_fold(Value::Unit, |acc, v| {
                    if matches!(acc, Value::Unit) {
                        Ok(v)
            ...
            Builtin::Pow => {
                ...
                match (&args[0], &args[1]) {
                    ...
                    (Value::Int(b), Value::Int(e)) => Ok(Value::Float((*b as f64).powi(*e as i32))),
            ...
            Builtin::Insert => {
                if args.len() != 3 {
                    return Err(error("insert expects 3 arguments (list, idx, value)", span));
                }
                match (&args[0], &args[1]) {
                    (Value::List(items), Value::Int(idx)) => {
                        let mut items = items.borrow_mut();
            ...
            Builtin::Remove => {
                if args.len() != 2 {
                    return Err(error("remove expects 2 arguments (list, idx)", span));
                }
                match (&args[0], &args[1]) {
                    (Value::List(items), Value::Int(idx)) => {
                        let mut items = items.borrow_mut();
            ...
            Builtin::Map => {
                if args.len() != 2 {
                    return Err(error("map expects (fn, list)", span));
                }
            ...
            Builtin::Filter => {
                if args.len() != 2 {
                    return Err(error("filter expects (fn, list)", span));
                }
                let func = args[0].clone();
                match &args[1] {
                    Value::List(items) => {
            ...
            Builtin::Reduce => {
                if args.len() != 2 {
                    return Err(error("reduce expects (fn, list)", span));
                }
                let func = args[0].clone();
                match &args[1] {
                    Value::List(items) => {
            ...
            Builtin::Enumerate => {
                ...
                match &args[0] {
                    ...
                    other => Err(error(
                        format!("enumerate: expected List or Range, got {other}"),
                        span,
                    )),
                }
            }
            Builtin::Zip => {
                if args.len() != 2 {
                    return Err(error("zip expects 2 lists", span));
                }
                match (&args[0], &args[1]) {
            ...
            Builtin::Range_ => match args.as_slice() {
                [Value::Int(n)] => {
                ...
                [Value::Int(a), Value::Int(b), Value::Int(step)] => {
                    if *step == 0 {
                        return Err(error("range step cannot be 0", span));
                    }
                    let mut v = Vec::new();
                    let mut i = *a;
                    if *step > 0 {
                ...
                _ => Err(error(
                    "range expects (n), (start, end), or (start, end, step)",
                    span,
                )),
            },
            Builtin::RangeStep => {
            ...
            // ── String operations ─────────────────────────────────────────────
            Builtin::Split => match args.as_slice() {
            ...
            Builtin::Join => match args.as_slice() {
            ...
            Builtin::StartsWith => {
                if args.len() != 2 {
                    return Err(error("starts_with expects (str, prefix)", span));
                }
                match (&args[0], &args[1]) {
            ...
            Builtin::EndsWith => {
                if args.len() != 2 {
                    return Err(error("ends_with expects (str, suffix)", span));
                }
                match (&args[0], &args[1]) {
            ...
            Builtin::Contains => {
                if args.len() != 2 {
                    return Err(error("contains expects (str, sub)", span));
                }
                match (&args[0], &args[1]) {
                    (Value::String(s), Value::String(sub)) => {
                        Ok(Value::Bool(s.contains(sub.as_str())))
                    }
                    (Value::List(items), v) => Ok(Value::Bool(
                        items.borrow().iter().any(|x| values_equal(x, v)),
                    )),
                    _ => Err(error(
                        "contains expects (String, String) or (List, value)",
                        span,
                    )),
                }
            }
            Builtin::Find => {
                if args.len() != 2 {
                    return Err(error("find expects (str, sub)", span));
                }
                match (&args[0], &args[1]) {
                    (Value::String(s), Value::String(sub)) => Ok(match s.find(sub.as_str()) {
                        Some(idx) => Value::Variant(
                            "Option".into(),
                            "Some".into(),
            ...
            Builtin::Replace => {
                if args.len() != 3 {
                    return Err(error("replace expects (str, old, new)", span));
                }
            ...
            Builtin::Substr => match args.as_slice() {
                ...
                _ => Err(error(
                    "substr(str, start[, end]) expects String and Int(s)",
                    span,
                )),
            },
            ...
            Builtin::Assert => match args.as_slice() {
                [cond] => {
            ...
            Builtin::DivMod => match args.as_slice() {
                [Value::Int(a), Value::Int(b)] => {
                    if *b == 0 {
                        return Err(error("divmod: division by zero", span));
                    }
            ...
            // ── Dict ──────────────────────────────────────────────────────────
            Builtin::Dict_ => Ok(Value::Dict(Rc::new(RefCell::new(HashMap::new())))),
            ...
            Builtin::ChannelNew => {
                let cap = match args.as_slice() {
                    ...
                    _ => {
                        return Err(error(
                            "Channel expects 0 or 1 non-negative Int argument",
                            span,
                        ))
                    }
                };
    ...
    fn eval_method_call(
        ...
        match (&tv, name) {
            ...
            (Value::List(items), "push") => {
                if arg_vals.len() != 2 {
                    return Err(error("push expects 1 argument", span));
                }
            ...
            (Value::List(items), "contains") => {
                if arg_vals.len() != 2 {
                    return Err(error("contains expects 1 argument", span));
                }
            ...
            (Value::List(items), "index") | (Value::List(items), "find") => {
                if arg_vals.len() != 2 {
                    return Err(error("index expects 1 argument", span));
                }
                let pos = items
                    .borrow()
                    .iter()
                    .position(|x| values_equal(x, &arg_vals[1]));
                return Ok(Flow::Value(match pos {
                    Some(i) => {
                        Value::Variant("Option".into(), "Some".into(), vec![Value::Int(i as i64)])
                    }
                    None => Value::Variant("Option".into(), "None".into(), vec![]),
                }));
            }
            ...
            (Value::List(items), "extend") => {
                if arg_vals.len() != 2 {
                    return Err(error("extend expects 1 argument", span));
                }
            ...
            (Value::List(items), "count") => {
                if arg_vals.len() != 2 {
                    return Err(error("count expects 1 argument", span));
                }
                let n = items
                    .borrow()
                    .iter()
                    .filter(|x| values_equal(x, &arg_vals[1]))
                    .count();
            ...
            (Value::List(items), "copy") => {
                return Ok(Flow::Value(Value::List(Rc::new(RefCell::new(
                    items.borrow().clone(),
                )))));
            }
            // String methods
            ...
            (Value::String(s), "strip" | "trim") => {
                return Ok(Flow::Value(Value::String(s.trim().to_string())))
            }
            (Value::String(s), "split") => {
                let parts: Vec<Value> = if arg_vals.len() == 2 {
                    match &arg_vals[1] {
                        Value::String(delim) => s
                            .split(delim.as_str())
                            .map(|p| Value::String(p.to_string()))
                            .collect(),
            ...
            (Value::String(s), "replace") => {
                if arg_vals.len() != 3 {
                    return Err(error("replace expects (old, new)", span));
                }
            ...
            (Value::String(s), "starts_with") => {
                if arg_vals.len() != 2 {
                    return Err(error("starts_with expects 1 argument", span));
                }
            ...
            (Value::String(s), "ends_with") => {
                if arg_vals.len() != 2 {
                    return Err(error("ends_with expects 1 argument", span));
                }
            ...
            (Value::Dict(map), "get") => {
                if arg_vals.len() != 2 {
                    return Err(error("get expects 1 argument", span));
                }
            ...
            (Value::Dict(map), "set") => {
                if arg_vals.len() != 3 {
                    return Err(error("set expects 2 arguments (key, value)", span));
                }
                let key = match &arg_vals[1] {
                    Value::String(s) => s.clone(),
                    v => v.to_string(),
            ...
            (Value::Dict(map), "remove" | "delete") => {
                if arg_vals.len() != 2 {
                    return Err(error("remove expects 1 argument", span));
                }
                let key = match &arg_vals[1] {
                    Value::String(s) => s.clone(),
                    v => v.to_string(),
            ...
            (Value::Dict(map), "len") => {
                return Ok(Flow::Value(Value::Int(map.borrow().len() as i64)))
            }
            (Value::Dict(map), "clear") => {
            ...
            (Value::Pointer(_, cell), "set") => {
                if arg_vals.len() != 2 {
                    return Err(error("set expects 1 argument", span));
                }
        ...
        match method {
            "send" => {
                ...
                if *chan.closed.borrow() {
                    return Err(error("send on closed channel", span));
                }
                if let Some(cap) = chan.capacity {
                    if chan.queue.borrow().len() >= cap {
                        return Err(error(format!("bounded channel at capacity ({cap})"), span));
                    }
                }
            ...
            "capacity" => Ok(Flow::Value(match chan.capacity {
                Some(c) => {
                    Value::Variant("Option".into(), "Some".into(), vec![Value::Int(c as i64)])
                }
                None => Value::Variant("Option".into(), "None".into(), vec![]),
            })),
    ...
    fn eval_binary(
        ...
        if op == BinaryOp::Or {
            let lv = self.eval_value(left)?;
            if value_is_truthy(&lv) {
                return Ok(Flow::Value(lv));
            }
            let rv = self.eval_value(right)?;
            return Ok(Flow::Value(rv));
        }
        let lv = match self.eval_expr(left)? {
            Flow::Value(v) => v,
            c => return Ok(c),
        };
        let rv = match self.eval_expr(right)? {
            Flow::Value(v) => v,
            c => return Ok(c),
        };
    ...
    fn eval_pipeline(&mut self, left: &Expr, right: &Expr, span: &Span) -> LangResult<Flow> {
        ...
        match right {
            ...
            _ => {
                let callee_v = match self.eval_expr(right)? {
                    Flow::Value(v) => v,
                    c => return Ok(c),
                };
        ...
        match callee {
            Value::Function(f) => {
                if let Some((lang, _)) = self.extern_names.get(&f.name).cloned() {
                    return Err(error(
                        format!(
                            "extern {lang} function '{}' is not callable in the interpreter",
                            f.name
                        ),
                        span,
                    ));
...
fn apply_binary(left: Value, op: BinaryOp, right: Value, span: &Span) -> LangResult<Value> {
    match (left, op, right) {
        ...
        // Mixed int/float
        (Value::Int(l), op, Value::Float(r)) => {
            apply_binary(Value::Float(l as f64), op, Value::Float(r), span)
        }
        (Value::Float(l), op, Value::Int(r)) => {
            apply_binary(Value::Float(l), op, Value::Float(r as f64), span)
        }
        ...
        (l, op, r) => Err(error(
            format!("unsupported operation: {l} {op:?} {r}"),
            span,
        )),
    }
}
